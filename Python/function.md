...
- 함수 선언 및 활용 시 인자 주의 사항
  - 선언 시 기본 인자 값을 가지는 인자는 기본 값이 없는 인자 뒤에 정의해야함

  - 호출 시 키워드 인자 다음에 위치 인자 활용 불가
    ...
    foo(26, name = '성재') # 됨
    foo(age = 26, '성재') # 안됨. 호출 시 키워드 인자 다음 위치 인자 나오면 안됨
    ```

    

  - 선언 시 가변 인자 리스트는 위치 인자보다 뒤에 정의해야함

  - 선언 시 가변 키워드 인자리스트는 위치 인자보다 뒤에 정의해야함
...
- 변수의 수명 주기
  ...
  - 전역 스코프 : 모듈 호출 후 또는 이름이 선언된 후부터 인터프리터가 끝나기 전까지 유지
